...
RSpec.describe GenerateRandomShips do
  ...
  context '#place_aircraft_carrier' do
    it "adds 'Aircraft Carrier' of 5 positions to board" do
  ...
  context '#place_battleship' do
    ...
    it "adds 'Battleship' of 4 positions to board" do
      expect(board.ships.length).to eq(1)
    ...
    it 'adds 4 more occupied_positions to board' do
      expect { @random_setup.place_battleship }
        .to change { board.occupied_positions.length }
        .from(5)
        .to(9)
    end
  ...
  context '#place_submarine' do
    ...
    it "adds 'Submarine' of 3 positions to board" do
      expect(board.ships.length).to eq(2)
    ...
    it 'adds 3 more occupied_positions to board' do
      expect { @random_setup.place_submarine }
        .to change { board.occupied_positions.length }
        .from(9)
        .to(12)
    end
  ...
  context '#place_destroyer' do
    ...
    it "adds 'Destroyer' of 3 positions to board" do
      expect(board.ships.length).to eq(3)
    ...
    it 'adds 3 more occupied_positions to board' do
      expect { @random_setup.place_destroyer }
        .to change { board.occupied_positions.length }
        .from(12)
        .to(15)
    end
  ...
  context '#place_patrol_boat' do
    ...
    it "adds 'Patrol Boat' of 2 positions to board" do
      expect(board.ships.length).to eq(4)
    ...
    it 'adds 2 more occupied_positions to board' do
      expect { @random_setup.place_patrol_boat }
        .to change { board.occupied_positions.length }
        .from(15)
        .to(17)
    end
