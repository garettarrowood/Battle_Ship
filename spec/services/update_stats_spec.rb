...
RSpec.describe UpdateStats do
  ...
  let(:game) do
    create(:game,
           users: [user, opponent],
           boards: [create(:board), create(:board, owner: user.id.to_s)])
  end
  ...
  describe '#user_wins' do
    ...
    it "game#winner is set to user's id" do
      expect { subject }.to change { game.winner }.to user.id.to_s
    end
  ...
  describe '#user_loses' do
    ...
    it "game#winner is set to opponent's id" do
      expect { subject }.to change { game.winner }.to opponent.id.to_s
    end
